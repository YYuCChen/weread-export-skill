...
核验报告会帮你检查是否存在缺失章节、串章、断头断尾或丢失图片。
...
对 Agent 说“重新登录微信读书”。Skill 会先备份旧登录状态，再打开浏览器让你重新扫码。

### 漫画或图片很多，会很费 Token 吗？

不会。图片数量、文件大小和文件格式都在你的电脑上检查，图片本身不会发给 Agent。
终端只显示一小段核验摘要，详细结果保存在 `核验报告.txt` 中。

图片很多时，下载和生成 PDF 会更久，最后得到的文件也会更大，这是正常现象。
